...
public static class FormJsonExtensions
{
	...
	public static string ToJson<TModel>(this Form<TModel> value, bool indented = false) where TModel : new()
		=> JsonSerializer.Serialize(value.Model, indented
	...
	/// <typeparam name="TModel">The model type to deserialize.</typeparam>
	/// <param name="json">The JSON string to deserialize.</param>
	/// <param name="value">When this method returns, contains the deserialized form instance if deserialization succeeded; otherwise, null.</param>
	/// <returns>True if deserialization succeeded; false if the JSON is invalid or deserialization failed.</returns>
	/// <exception cref="ArgumentNullException"><paramref name="json"/> is null.</exception>
	public static bool TryFromJson<TModel>(string json, [System.Diagnostics.CodeAnalysis.NotNullWhen(true)] out Form<TModel>? value) where TModel : new()
	{
		ArgumentNullException.ThrowIfNull(json);
